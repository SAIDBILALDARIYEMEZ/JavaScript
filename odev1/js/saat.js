...
function tarihSaat() {
    var tarihOptions = {weekday: 'long', year: 'numeric', month: 'long', day: 'numeric'};
    var saatOptions = {hour: 'numeric', minute: 'numeric', second: 'numeric'};

    var now = new Date();
    document.getElementById("tarih").textContent = now.toLocaleString('tr-TR', tarihOptions);
    document.getElementById("saat").textContent = now.toLocaleString('tr-TR', saatOptions);
...
function hesapla() {
    ...
    function tekrar(){
        zamanCevrimi.yil !== 0 ? yilElement.innerText = "yıl\n" + zamanCevrimi.yil : ""
        zamanCevrimi.ay !== 0 ? ayElement.innerText = "ay\n" + zamanCevrimi.ay : ""
        zamanCevrimi.gun !== 0 ? gunElement.innerText = "gün\n" + zamanCevrimi.gun : ""
        zamanCevrimi.saat !== 0 ? saatElement.innerText = "saat\n" + zamanCevrimi.saat : ""
        zamanCevrimi.dakika !== 0 ? dakikaElement.innerText = "dakika\n" + zamanCevrimi.dakika : ""
        zamanCevrimi.saniye !== 0 ? saniyeElement.innerText = "saniye\n" + zamanCevrimi.saniye : ""
    }

    //said if else code
    if (girilenTarih > suAnkiTarih) {
        sonuc.textContent = "Gelecek bir tarih girdiniz. Girilen tarihe kalan zaman:";
        tekrar()
    } else if (girilenTarih < suAnkiTarih) {
        sonuc.textContent = "Geçmiş bir tarih girdiniz. Girilen tarihten geçen zaman:";
        tekrar()
    } else {
